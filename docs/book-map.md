...
`scripts/build_image.sh` は p1（boot / FAT32） + p2（rootfs / ext4、既定 160 MB） +
p3（data / ext4、残り全部）の 3 パーティションを作成します。

本文の 3.3 では p1+p2 の 2 パーティションとして説明し、p3 は 5.2 で
「ここに `/data` 用の p3 を足します」として導入する段階的な構成になって
