...
#
# 既存 scripts/build_bundle.sh(cross offline-root)は温存。ct.sh は postinst 実行・
# 対話探索・ソースビルドが要る場合の上位路線。
set -eu
...
gen_manifest() { # gen_manifest <name> <image>
  ...
  {
    echo "# generated by ct.sh build from Dockerfile LABEL ct.* — 手で編集しない"
    echo "exec=\"$ex\""
...
# built イメージと base の rootfs を書き出し、差分(追加/変更)だけを root/ へ抽出
extract_delta() { # extract_delta <image> <out-root> [clean=1]
  ...
  # clean=1(build): root/ を Dockerfile から完全再生成。clean=0(shell): 既存に merge。
  [ "$clean" = 1 ] && rm -rf "$out"
  mkdir -p "$out"
  # import 由来イメージは CMD が無く docker create が拒否するのでプレースホルダを渡す(起動はしない)。
  ...
  # base と同一のファイルは除外(--compare-dest)。実行時に lower から来る/不要なパスも除外。
...
# 対話コンテナを commit → 差分を root/ に merge 取り込み(build と共通の抽出)
...
cmd_shell() {
  ...
  # CT_SHELL_CMD が設定されていれば非対話で実行(スクリプト化ビルド/テスト用)
