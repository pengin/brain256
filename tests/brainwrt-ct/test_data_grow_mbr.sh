#!/bin/sh
# patch_mbr() のバイト単位検証。MBR 相当のダミーファイルにセンチネル値を書いておき、
# patch_mbr 実行後にオフセット 490～493 がリトルエンディアンで期待値になるか確認する。
set -eu
...
root=$(mktemp -d)
disk="$root/fake-disk"
# 512 バイトのダミー MBR。全体を 0xAA（センチネル）で埋める。
dd if=/dev/zero bs=1 count=512 2>/dev/null | LC_ALL=C tr '\0' '\252' > "$disk"

export DISK_DEV="$disk"

# 0x01020304 = 16909060 をパッチする。リトルエンディアンで 04 03 02 01 になるはず。
sh -c ". '$SCRIPT'; patch_mbr 16909060"

# macOS（BSD od）と Linux（GNU od）では `od` の出力フォーマット（端数行のパディング
# など）が異なるため、テキスト比較ではなく `cmp` でバイト単位に比較する。
