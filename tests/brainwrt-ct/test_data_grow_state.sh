#!/bin/sh
# brainwrt-data-grow の boot() 状態機械を検証する。実ブロックデバイス／実 sysfs は
# 使えないため、DISK_DEV などの env var seam + PATH スタブ + マーカーファイルで
# 各遷移を確認する。
set -eu
...
# 99000 = 0x000182B8 -> LE: b8 82 01 00（cmp でバイト単位に比較して od の
