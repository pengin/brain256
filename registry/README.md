...
> **重要: 同人誌の教材・テスト専用です。** `brain-registry` は本の手順を再現するための最小実装であり、本番運用のレジストリやインターネット向けの配布サービスではありません。ユーザー認証、アクセス制御、HTTPS、署名検証などを備えていません。`PUSH_TOKEN` は `PUT` の送信時に使う共有トークンにすぎず、`GET` は認証なしで実行できます。このサーバーをインターネットから到達できる場所へ、絶対に直接公開しないでください。信頼できる LAN または VPN などの閉じたネットワーク内で行う教材・テストにだけ使用してください。
...
| `PUSH_TOKEN` | なし | 必須の送信認証トークンです。未設定または空文字の場合、起動に失敗します。文字種・長さ・形式の検証はありません。 |

### `PUSH_TOKEN` の制約

サーバーが確認するのは、`PUSH_TOKEN` が空文字でないことだけです。文字種、長さ、接頭辞、ランダム性などの検証は行いません。`PUT` の認証では、HTTP の `Authorization` ヘッダーが次の文字列と完全一致するかを比較します。

```text
Bearer <PUSH_TOKEN>
```

したがって、空白や記号を含むトークンも、クライアントが同じ値を HTTP ヘッダーとして送信できれば使用できます。ただし、改行など HTTP ヘッダーに使用できない制御文字は、サーバーのトークン検証ではなく HTTP 実装や環境側の制約で使用できません。運用上は、HTTP ヘッダーで安全に扱える十分に長いランダムな ASCII 文字列を指定してください。
