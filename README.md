...
ビルド済みの SD イメージを [リリース](https://github.com/pengin/brain256/releases) に置いてあります。まず動く状態を見たい場合は、こちらが最短です。Docker も OpenWrt ImageBuilder も要りません。

1. リリースから `brain256-sd-<日付>.zip` をダウンロードし、展開して `brainwrt-sd.img` を取り出します。
2. 2GB 以上の Micro SD カードへ書き込みます。Raspberry Pi Imager や balenaEtcher でも、`dd` でも構いません。

   ```sh
   # macOS の例。diskN は必ず diskutil list で確認してください。
   # 間違えると別のディスクを破壊します。
   diskutil unmountDisk /dev/disk4
   sudo dd if=brainwrt-sd.img of=/dev/rdisk4 bs=4m
   diskutil eject /dev/disk4
   ```

3. Brain に挿し、メニューから「アプリ」→「Launch Linux」を選びます。
4. USB ケーブルでホスト PC とつなぐと、Brain が USB Ethernet 機器として見えます。ホスト PC 側のインターフェースへ IP を振って SSH します（`root`、パスワードなし）。

   ```sh
   sudo ifconfig en5 192.168.28.1 netmask 255.255.255.0 up   # en5 は環境で変わります
   ssh root@192.168.28.2
   ```

このイメージについて、次の点をご承知おきください。

- **PW-SH1 から PW-SH7 のどれでも起動します。** 機種ごとにイメージを分けていません。BrainLILO が実機の型番を見て、対応する U-Boot を選びます。
- **イメージは 2048 MiB です。** 市販の「4GB」カードは実容量が 3.7 GiB 程度しかないため、既定のビルド設定（4096 MiB）では入りません。リリース物はどの 4GB カードにも収まる大きさにしてあります。
- **データ領域は SD カードの実容量まで自動では広がりません。** 大きなカードを使い切りたい場合は[データ領域（p3）の大きさ](#データ領域p3の大きさ)を参照してください。
- ダウンロードしたファイルは、リリースページに記載の sha256 と照合してください。

自分でビルドする場合は次へ進んでください。設定を変えられるほか、何が起きているかを追えます。
